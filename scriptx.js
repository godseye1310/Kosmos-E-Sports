...
{
	let arr_image = ["a-i0", "a-i1", "a-i2", "a-i3"];
	let i = 0;
	...
	function nextImage() {
		...
		setTimeout(() => {
			document.getElementById(
				"box"
			).innerHTML = `<img src="images/${arr_image[i]}.jpg" alt="Team Kosmos eSports">`;
	...
	function prevImage() {
		...
		setTimeout(() => {
			document.getElementById(
				"box"
			).innerHTML = `<img src="images/${arr_image[i]}.jpg" alt="">`;
			document.getElementById("box").style.opacity = 1;
		}, 300);
	}
...
// FORM VALIDATION //
{
	...
	function validate() {
		...
		if (x.length < 10 || x.length > 13) {
			document.getElementById("error-phone").innerHTML =
				"Contact No. should be between 10 & 13 digits";
			document.getElementById("phone").focus();
			return false;
		} else {
			document.getElementById("error-phone").innerHTML = "";
		...
		if (x == "") {
			...
			document.getElementById("error-address").innerHTML =
				"Type Address in correct format";
			document.getElementById("user-address").focus();
			return false;
		} else {
			document.getElementById("error-address").innerHTML = "";
